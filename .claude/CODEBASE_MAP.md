...
- Path alias `@/*` → `./src/*`. **[v]**
- Fonts: **Plus Jakarta Sans** (brand sans, `--font-brand-sans`, mapped to `--font-sans` in globals.css; chosen 2026-09-18, the user asked for a font change without naming one) + Geist Mono for mono labels, via `next/font/google` (fetched at build time, so it needs network). Headings still use the tight `tracking-[-0.035em…-0.04em]` tuned for Geist. **[v]**
...
- `src/shared/` — feature-agnostic code: `ui/` (`button-link`, `brand-logo` (real logo: header + footer), `brand-mark` (placeholder shape still used in the integrations tile), `new-badge`, `section-heading` (optional `titleWidth`, default `max-w-[20ch]`; titles md `text-[2rem] sm:text-[2.5rem]`, lg `text-[2.25rem] sm:text-[2.875rem]`; description `max-w-lg text-base leading-7`, zinc-600 light / zinc-300 dark), `integration-logo` + `IntegrationId`), `lib/cn.ts`, `types.ts` (`Cta`)
...
## Conventions
- **Section width:** all content containers (Problem, Services heading, Workflow, Selected work, Technology, FAQ, footer) use **`max-w-[80rem]`** (1280px, which lines up exactly with the header bar `max-w-7xl`; user 2026-09-18, after an earlier widening to 88rem); the hero keeps its own narrower text column. Section eyebrows are a `text-sm font-medium text-brand-700` `<p>` above `SectionHeading` (`text-brand-300` on ink).
...
- Services is an **ink section**: heading in the same `max-w-[80rem]` container as Problem; the infinite CSS marquee sits **outside** it, edge to edge (`-mx-4`, no edge fade): two copies, -50% loop, `animationDuration: 60s` inline, per-card `pr-4` (never `gap`), pause on hover, edge mask; reduced motion gives one manually scrollable row. The duplicate list is `aria-hidden` and omits heading ids. `SectionHeading` takes `tone="dark"` for ink backgrounds.
...
- **Type scale (2026-09-18):** all sections except the hero were bumped one Tailwind text step (e.g. 14→15, 15→16px) with body greys one step more contrasty (zinc-500→600 on light, zinc-400→300 on ink). Illustration mocks (automation-flow, app-mocks, flow-canvas cards, the featured-project diagram) and the header were left at their own sizes.
