...
- `src/features/<feature>/` — feature code. Each feature has an `index.ts` public API; import from that, never from its internals
  ...
  - **Client components (all of them):** `nav-dropdown`, `mobile-nav`, `auto-hide-header` (hides the fixed header on scroll down and shows it on scroll up; 8px direction threshold, only after 120px; not while any `[aria-expanded=true]` menu is open; focus-within reveals it), `workflow/components/workflow-tabs`
